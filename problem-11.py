...
def main():
    ...
    for arr in temp_strings:
        data_array.append(arr.split(" "))
    product = 0
    # diagonals: left down to right
    for column in range(0, 17):
        for row in range(0, 17):
            first = int(data_array[row][column])
            second = int(data_array[row+1][column+1])
            third = int(data_array[row+2][column+2])
            forth = int(data_array[row+3][column+3])
            temp = first * second * third * forth
            if temp > product:
                product = temp

    #diagonals: right down to left
    for column in reversed(range(3, 20)):
        for row in range(0, 17):
            first = int(data_array[row][column])
            second = int(data_array[row+1][column-1])
            third = int(data_array[row+2][column-2])
    ...
    for column in range(0, 17):
        for row in range(0, 20):
            ...
            forth = int(data_array[row][column+3])
            temp = first * second * third * forth
            if temp > product:
                product = temp
